...
class Driver(LabberDriver):
    ...
    def performOpen(self, options={}):
        ...
        self._scope_triggered = False
        self._scope_waittime = 0.05
        self._output_device = False # Flag the device as output device/acquisition device
    ...
    def performSetValue(self, quant, value, sweepRate=0.0, options={}):
        ...
        elif "Scope" in quant.name and quant.name not in ["Scope - Channel 1 Input", "Scope - Channel 2 Input"]:
            if quant.set_cmd:
                value = self.set_scope_value(quant, value)
            if quant.name == "Scope - Measure" and value:
                self.scope_measure()
            if quant.name == "Scope - Waittime" and value:
                self._scope_waittime = value
    ...
    def performGetValue(self, quant, options={}):
        ...
        # getting a value from the Sweeper module
            ...
            # if self.controller.scope.enable() == 1:
            #     self.controller.scope.enable(0)
            if quant.get_cmd:
                return self.get_scope_value(quant)
            elif "Trace" in quant.name:
                try:
                    if self._scope_triggered:
                        self.controller.scope.acquire()
                        self._scope_triggered = False
                    result = self.controller.scope._result
                    return self.scope_result_to_quant(quant, result)
                except:
                    return self.scope_return_zeros(quant)
            elif "Waittime" in quant.name:
                return self._scope_waittime
    ...
    def performArm(self, quant_names, options={}):
        ...
        if self._output_device is False:
            if self.controller.scope.enable() == 0:
                self.controller.scope.enable(1)
                time.sleep(2)
            self.controller.scope.armtrigger()
            self._scope_triggered = True
